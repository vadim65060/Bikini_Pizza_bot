...
def profile_text_markup_get(db: AdminDataBase, tg_id, cancellation=False):
...
def merch_issue_count_text(db: AdminDataBase, purchases_id, k):
    ...
    size_color = db.get_stuff_sizes_texts_info(sizes_colors_id, "size, color")
...
def merch_return_back_count_text(db: AdminDataBase, purchases_id, k):
    tg_id, fullname, stuff_name, sizes_colors_id, count, issued = db.execute(
        "SELECT users.tg_id, "
        "users.fullname, "
        "stuff.name, "
